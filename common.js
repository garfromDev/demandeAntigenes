...
/**
* perform the function that is written in remark in the cell
* @param {object} e: parameter from onEdit()
* @return : none
*/
function performNoteAction(e) {
  const r=e;
...
/**
 * when the box is checked (cell value is true), write user account and date in the 2 columns 
 * at the right of the cell e
 * @param {object} e: parameter from onEdit()
 * @param {string} sheet_to_check : name of the sheet where the checkbox are
 * @param {string} column_to_check : name of the column (like "B")
 * @param {[function]} actions : list of function to call, e will be passed in parameter
 * @return : none
 */
function checkbox(e, sheet_to_check, column_to_check, actions) {
  // Verification de la feuille
  if(sheet_to_check != ""){
    var shObj = e.range.getSheet();
    var sh = shObj.getName();
    if(sh != sheet_to_check) { return; }
  }
  // vérification de la colonne
  const col = e.range.getColumn(); 
  if(e.range.getValue() && (columnToLetter(col) == column_to_check)) {
